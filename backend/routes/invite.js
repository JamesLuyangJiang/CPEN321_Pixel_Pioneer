...
async function inviteSpecifiedUser(req, res) {
  ...
  if (isConnected) {
    try {
      ...
      const checkEventExists = await eventsCollection.findOne({ userid, name: req.body.name });
      if (!checkEventExists) {
      ...
      const checkReceiverEmailExists = await usersCollection.findOne({ email: req.body.receiver });
