...
async function fetchAstronomyInfoOnScheduledDate(observatory_name, date) {
    ...
    try {
      const astronomy_response = await axios.get(
        `http://api.weatherapi.com/v1/astronomy.json?key=${CONFIDENTIAL_WEATHER_API_KEY}&q=name=${observatory_name}&date=${date}`);
      const astronomy_data = {
         sunrise: astronomy_response.astronomy.astro.sunrise,
         sunset: astronomy_response.astronomy.astro.sunset
      }
    } catch (err) {
      item.weatherForecast = [];
      console.error("Call astronomy API failed...");
    }
