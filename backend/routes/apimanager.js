...
async function fetchNearbyObservatoryFromAPIs(radius) {
    const client_ip = "184.68.183.186"; // hardcoded for testing purpose, TODO: need to replace with req.ip in the future
      try {
        ...
        nearby_observatory_list = await fetchNearbyObservatoriesList(lat, lon, radius);
        var ObservatoriesWithConditionInfo = [];
        ObservatoriesWithConditionInfo = await fetchObservatoriesWithConditionInfo(nearby_observatory_list);
...
async function fetchObservatoriesWithConditionInfo(nearby_observatory_list) {
    const weatherAPIRequests = nearby_observatory_list.map(async (item) => {
            try {
              const weather_forecast_response = await axios.get(
                `http://api.weatherapi.com/v1/forecast.json?key=${CONFIDENTIAL_WEATHER_API_KEY}&q=${item.latitude},${item.longitude}&days=10&aqi=yes&alerts=no`);
              const weather_data =
    ...
    await Promise.all(weatherAPIRequests).then(() => {
    }).catch(error => {
